...
HRESULT Sample::LoadShader()
{
    ...
    hr = D3DCompileFromFile(L"PixelShader.txt", nullptr, nullptr, "PS", "ps_5_0", 0, 0, &m_PSblob, nullptr);
    if (FAILED(hr))
    {
        return hr;
    }
    hr = m_pd3dDevice->CreatePixelShader(m_PSblob->GetBufferPointer(), m_PSblob->GetBufferSize(), NULL, &m_pPS);
