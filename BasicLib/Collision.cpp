...
collisionRet Collision::RectToRect(Rect rt1,
	Rect rt2)
{
	Rect rtInterction;
...
bool Collision::RectToPoint(Rect rt, HVector2 v)
{
...
collisionRet   Collision::ToRect(Rect rt1, Rect rt2)
{
	// 거리 판정
	float fDistanceX;
	float fDistanceY;
...
bool   Collision::BoxToPoint(Box rt, HVector3 v)
{
	if (rt.vMin.x <= v.x && rt.vMax.x >= v.x &&
		rt.vMin.y <= v.y && rt.vMax.y >= v.y &&
...
// 1 :  안에 있다.
// 2 :  걸쳐 있다.
collisionRet   Collision::BoxToBox(Box rt1, Box rt2)
{
	Box rtInterction;
